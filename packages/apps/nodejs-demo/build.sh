...
REGISTRY="${REGISTRY:-ghcr.io/mrsimpson}"
...
log_info "Building ${APP_NAME} using ${CONTAINER_CMD}..."
...
if [[ "${PUSH:-false}" == "true" ]]; then
    ...
    if ${CONTAINER_CMD} info 2>&1 | grep -q "ghcr.io"; then
        ...
        if command -v gh &> /dev/null; then
            ...
            if [[ -n "${GH_TOKEN}" && -n "${GH_USER}" ]]; then
                echo "${GH_TOKEN}" | ${CONTAINER_CMD} login ghcr.io -u "${GH_USER}" --password-stdin
            ...
            echo "${GITHUB_TOKEN}" | ${CONTAINER_CMD} login ghcr.io -u "${GITHUB_USERNAME}" --password-stdin
            ...
            echo "  2. Run: ${CONTAINER_CMD} login ghcr.io"
            echo ""
            read -p "Press Enter after logging in, or Ctrl+C to cancel..."

            # Verify login worked
            if ! ${CONTAINER_CMD} info 2>&1 | grep -q "ghcr.io"; then
                log_error "Authentication failed. Please run: ${CONTAINER_CMD} login ghcr.io"
                exit 1
            fi
        fi
    fi
...
# ============================================================================
log_step "Step 1/4: Building container image..."
${CONTAINER_CMD} build \
    --platform "${PLATFORMS}" \
...
if [[ "${SKIP_TEST:-false}" != "true" ]]; then
    ...
    CONTAINER_ID=$(${CONTAINER_CMD} run -d --rm -p 3000:3000 "${IMAGE_NAME}:${VERSION}")
...
if [[ "${PUSH:-false}" == "true" ]]; then
    ...
    if [[ "${GIT_SHA}" != "dev" ]]; then
        ${CONTAINER_CMD} push "${IMAGE_NAME}:${GIT_SHA}"
    fi
...
echo ""
echo "Next steps:"
echo "  1. Run locally:  ${CONTAINER_CMD} run -p 3000:3000 ${IMAGE_NAME}:${VERSION}"
echo "  2. Deploy:       cd deployment && pulumi up"
echo "  3. Access:       https://nodejs-demo.yourdomain.com"
